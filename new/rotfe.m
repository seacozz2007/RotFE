...
%	Rot=rotfe('simple1s') 
%	spy(Rot.M)
%        ... called indirectly by modeplot2,  example:     modeplot2 simple1s
%   
% By  I. Bucher
% Date 21-8-1998   
% Rev. 3.01
% 28-8-98  Added Speed depeendent pseudo stiffness Kst
...
if isa(File,'struct'); % input was a Rot structure
   ...
   try, run(File)
   catch, error(['rotfe:  Errors in script file >> ' File ' << or file not found in MATLAB path']);   
   end
   save_flag=nargin > 1;  
end
...
cmd='clear nmx nmy tmp  R nd md BC_DIR cmd m Ai Jp Jd drho dmat drhi t ext Me Ge Ke De KHe nod ro ri L  Nelem FAIL dof4nod n1 n2 Cd E nu rho q dro dri nd1 cord1y cord1z tMATERIALS dof1 mat_no Kste cord Kb cord1yz mu  nm nod1 f n';
