...
elseif strcmp(FILE,'modeplus') 
   ...
   set(hed,'value',n);   modeplot('display')
   %>>>>>>>>>>>>>>>>>>>>>>>
...
elseif  strcmp(FILE,'display'),
   ...
   if p3D,
      ...
      h=plot(x,real(Wu)*scl,'r-',x, real(Wv)*scl,'m-'	,[min(x)-dx/s max(x)+dx/s],[0 0],'k-.');
...
elseif strcmp(FILE,'newfile')
   h = findobj(gcf,'tag','modeplotfile');
   FILE=get( h, 'string');
   try
   ...
   uicontrol( 'Value',1, ...
      ...
      'String','Show plane' , ...
      'Style','radiobutton', ...
      'ToolTip',['Show a plane connecting the updeformed shaft' 'to the deformed state'],...
      'callback',[tool '(''display'');'] ,...
      'tag',[tool 'plane']);  
