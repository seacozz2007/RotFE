...
for q=1:ns,
   ...
   v=R1.SPRINGS(1,2:5); v=[v(1) v(3);v(4) v(2)];
   [T dd]=eig(v); dd=sqrt(diag(dd)); dd=dd/max(dd);
   h1=line([x1 x1],[-Do_max Do_max]/2 ,[0 0]);
   h2=line([x1 x1],[0 0],[-Do_max Do_max] /2);
  
   hspr1=plot3(x1+D3*cos(ncoils*th),z*Lt,D3*sin(ncoils*th));
   hspr2=plot3(x1+D3*cos(ncoils*th),-z*Lt,D3*sin(ncoils*th));
   hspr3=plot3(x1+D3*sin(ncoils*th),D3*cos(ncoils*th),z*Lt);
   hspr4=plot3(x1+D3*sin(ncoils*th),D3*cos(ncoils*th),-z*Lt);
     set([hspr1(:);hspr2(:);hspr3(:);hspr4(:)],'linewidth',2);
