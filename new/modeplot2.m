function modeplot2(FILE,arg1,arg2)  
% [phi,lambda,fb]=modeplot2(FILE,W1,W2,N,P5) 
%   
%
% INPUTS:
...
elseif strcmp(FILE,'modeplus') 
   ...
   set(hed,'value',n);   modeplot2('display')
   %>>>>>>>>>>>>>>>>>>>>>>>
...
elseif  strcmp(FILE,'display'),
   ...
   if p3D,
     ...
     rotmshape3a(Rot.eigenvector(:,n),Rot,NP,pQ,cQ);
      ...
      scl=0.3*mx/max(abs([eps;real(Wu(:)); real(Wv(:))]));
      h=plot(x,real(Wu)*scl*1.5,'r-',x, real(Wv)*scl*1.5,'m-'	,[min(x)-dx/s max(x)+dx/s],[0 0],'k-.');
...
elseif strcmp(FILE,'newfile')
   h = findobj(gcf,'tag','modeplot2file');
   FILE=get( h, 'string');
   try
   ...
   if length(fig)==0, 
      fig=figure('unit','normal','pos',...
         ...
         'name','mode shapes',...
         'numbertitle','off'); 
     % OPENGL NEVERSELECT, set(gcf,'renderer','zbuffer')
